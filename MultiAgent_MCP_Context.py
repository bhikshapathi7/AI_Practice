...
async def main():
    ...
    try:
        ...
        agent_factory = AgentFactory(model_client)
        db_agent = agent_factory.dataBase_Agent(system_message="""You are a Database specialist responsible for retrieving user registration data.
            ...
            """ )
        rest_agent = agent_factory.restAssured_agent(system_message="""You are an API testing specialist with access to both REST API tools and filesystem.
            ...
            """)
        excel_agent = agent_factory.excelFile_agent(system_message="""You are an Excel data management specialist. ONLY proceed when APIAgent has completed testing.
        ...
        team = RoundRobinGroupChat(participants=[db_agent,rest_agent,excel_agent],
                               termination_condition=TextMentionTermination("Execute Sequential User Registration Process:\n\n"
        ...
        await Console(team.run_stream(task="Fetch data from DB, validate via API, and store results in Excel"))
